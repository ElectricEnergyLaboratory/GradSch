...
計算の流れとしては、まず `pw.x` によるSCF計算を通じて電子密度の収束を確認。その後、同じ電子密度を用いて `bands.x` によるバンド構造の算定を行った。PDOS（Projected Density of States）については、`projwfc.x` を用いた非自己無撞着（NSCF）計算を一部実施したが、全原子種に対する解析は未完了である。可視化については、従来の `gnuplot` は使用せず、**xfroggie** の描画機能を活用してバンド構造図およびDOSプロットを生成した。xfroggieでは、計算結果のアップロード後に自動的に分散図が描画され、対称点ラベルやエネルギー軸の調整も可能であるため、手元でのスクリプト編集は不要だった。
